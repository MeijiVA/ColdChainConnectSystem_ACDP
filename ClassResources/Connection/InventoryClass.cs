    ...
    internal class InventoryClass
    {
        ...
        public static int loadInventoryData(DataGridView dgv, Label lblPage, Label lblPageNum, int currentPageIndex, string searchQuery)
        {
            ...
            using (SqlCommand count = new SqlCommand(query, con))
            {
                ...
                totalPages = (int)Math.Ceiling((double)totalRows / PageSize);
                lblPage.Text = totalPages.ToString();
                query = $"SELECT numid,skucode, descript, image, unitprice, kg, quantity, expiry FROM Inventory ORDER BY numid OFFSET {(currentPageIndex - 1) * PageSize} ROWS FETCH NEXT {PageSize} ROWS ONLY {searchQuery}";
                using (SqlDataAdapter data = new SqlDataAdapter(query, con))
                {
