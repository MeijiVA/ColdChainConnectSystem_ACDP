    ...
    internal class CustExcelClass
    {
        ...
        public static void Export(String searchQuery, String ofd)
        {
            ...
            try
            {
                ...
                {
                    query = $"WHERE [numid] LIKE '%{(searchQuery)}%' OR [CustomerID] LIKE '%{(searchQuery)}%' OR [CustomerName] LIKE '%{(searchQuery)}%' OR [PhoneNumber] LIKE '%{(searchQuery)}%' OR [Address] LIKE '%{(searchQuery)}%' OR [PaymentTerm] LIKE '%{(searchQuery)}%' OR [RegistrationDate] LIKE '%{(searchQuery)}%' OR [Status] LIKE '%{(searchQuery)}%' OR ";
                }
                else
                {
        ...
        //not mine, don't edit xd (nvm it's mine now mfs)
                ...
                using (SqlConnection connection = ConnectionClass.Connection())
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            ...
                            excelWorksheet.Cells[1, 1].Value = "Customer";
                            int col = 1;
                            ...
                            while (reader.Read())
                            {
                                ...
                                for (int i = 0; i < reader.FieldCount; i++)
                                {
                                    if (i == 6)
                                    {
                                        DateTime date = Convert.ToDateTime(reader[6].ToString());
